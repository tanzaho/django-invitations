# -*- coding: utf-8 -*-
from django_rq import job, enqueue, get_scheduler, get_queue, get_scheduler
...
def resend_inactive_invitations(invitation_id, ctx):   
    ...
    scheduler.enqueue_in(timedelta(days=app_settings.INVITATION_EXPIRY - 1), send_reminder, ctx['email'], 'invitation_reminder_24hrs', 'Your invitation is expiring today', ctx, invitation_id)
    scheduler.enqueue_in(timedelta(days=app_settings.INVITATION_EXPIRY - 2), send_reminder, ctx['email'], 'invitation_reminder_48hrs', 'Your invitation is expiring tomorrow', ctx, invitation_id)
...
def send_reminder(email, newsletter_name, title, ctx, invitation_id):
    ...
    # Sending email
    email = EmailMessage(title, html_content, settings.JICE_FROM_EMAIL, [email])
